...
import { Choice, getQuestion, getSetForQuestion } from "@/data/toeic/test1/reading";
import { gradeToeicReading } from "./grade";
import { ReadingResultDetailPanel } from "./ReadingResultDetailPanel";
...
const PART_LABELS: Record<5 | 6 | 7, string> = {
  5: "Part 5",
  6: "Part 6",
  7: "Part 7",
};
...
const ALL_QUESTION_IDS = Array.from({ length: 100 }, (_, i) => i + 101);
...
export function ReadingResultsScreen({
  ...
  const grade = useMemo(() => gradeToeicReading(answers), [answers]);
  ...
  const selectedSet = selectedId ? getSetForQuestion(selectedId) : undefined;
  ...
  return (
    <Container className="py-10">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-6">
        <div>
          <h1 className="text-headline-lg text-primary mb-2">TOEIC Reading Test 1 — Results</h1>
        ...
        <div className="border border-outline-variant bg-secondary-container px-6 py-4">
          ...
          <p className="text-body-sm text-on-secondary-container mt-1">
            Estimated RC score: {grade.scaledScore} / 495
          </p>
        </div>
      </div>
