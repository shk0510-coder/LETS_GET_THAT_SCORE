...
import { ToeicSectionId, TOEIC_PRACTICE_TYPES, TOEIC_SECTIONS } from "@/lib/content";

export function PracticeByTypeClient() {
  const [activeSection, setActiveSection] = useState<ToeicSectionId>("listening");
  const [activeTypeIndex, setActiveTypeIndex] = useState(0);

  const types = TOEIC_PRACTICE_TYPES[activeSection];
  ...
  return (
    <>
      <Tabs
        ...
        onChange={(id) => {
          setActiveSection(id as ToeicSectionId);
        ...
        {Array.from({ length: activeType.count }, (_, i) => i + 1).map((n) => {
          ...
          return isReady ? (
            <TestTile
              ...
              number={n < 10 ? `0${n}` : `${n}`}
              label={`Practice ${n}`}
              href={`/toeic/practice/reading/part5/${n}`}
            />
          ) : (
            <TestTile
              key={n}
